...
describe Dash::TransactionBuilder do

  class SignerByKey
    include Dash::TransactionBuilder::Signer
  ...
  describe "TransactionBuilder with no outputs" do
    before do
      @builder = Dash::TransactionBuilder.new
      @all_utxos = self.mock_utxos

      @builder.input_addresses = self.mock_addresses
      @builder.provider = Dash::TransactionBuilder::Provider.new do |txb|
      ...
      @builder.change_address = Dash::Address.parse("1CBtcGivXmHQ8ZqdPgeMfcpQNJrqTrSAcG")
    end
    ...
    it "should have a default fee rate" do
      @builder.fee_rate.must_equal Dash::Transaction::DEFAULT_FEE_RATE
    end

    it "should return a result" do
      result = @builder.build
      result.class.must_equal Dash::TransactionBuilder::Result
    end
    ...
    it "should have a reasonable fee" do
      ...
      result.fee.must_be :>=, (size / 1000)*Dash::Transaction::DEFAULT_FEE_RATE
      result.fee.must_be :<, 0.01 * Dash::COIN
    end
    ...
    it "should sign if signer provides a signature script" do
      @builder.signer = SignerBySignatureScript.new do |txin, txout|
        Dash::Script.new << "signature" << "pubkey"
      end
      result = @builder.build
      result.unsigned_input_indexes.must_equal []
      result.transaction.inputs.each do |txin|
    ...
    it "should include all prepended unspents AND all normal unspents" do
      @builder.prepended_unspent_outputs = [ Dash::TransactionOutput.new(
        value: 6_666_666,
        script: Dash::Address.parse("3EktnHQD7RiAE6uzMj2ZifT9YgRrkSgzQX").script,
        index: 0,
  ...
  describe "TransactionBuilder with some outputs" do
    before do
      ...
      @builder.change_address = Dash::Address.parse("1CBtcGivXmHQ8ZqdPgeMfcpQNJrqTrSAcG")
      @builder.outputs = [ Dash::TransactionOutput.new(value: 1000_500,
                            script: Dash::Address.parse("1TipsuQ7CSqfQsjA9KU5jarSB1AnrVLLo").script) ]
    end
  ...
  describe "TransactionBuilder edge cases" do
    ...
    it "should detect missing change address" do
      ...
      lambda do
        result = @builder.build
      end.must_raise Dash::TransactionBuilder::MissingChangeAddressError
    end
    ...
    it "should detect missing unspents" do
      ...
      lambda do
        result = @builder.build
      end.must_raise Dash::TransactionBuilder::MissingUnspentOutputsError
    end
    ...
    it "should detect not enough unspents" do
      @builder.outputs = [ Dash::TransactionOutput.new(value: 100*Dash::COIN, script: @builder.change_address.script) ]
      lambda do
        result = @builder.build
      end.must_raise Dash::TransactionBuilder::InsufficientFundsError
    end
    ...
    it "should detect not enough unspents because of change constraints" do
      ...
      @builder.outputs = [ Dash::TransactionOutput.new(value: 1000_00 - @builder.fee_rate - 10, script: @builder.change_address.script) ]
      lambda do
        result = @builder.build
      end.must_raise Dash::TransactionBuilder::InsufficientFundsError
    end
    ...
    it "should forgo change if it's below dust level" do
      ...
      @builder.outputs = [ Dash::TransactionOutput.new(value: amount, script: @builder.change_address.script) ]
  ...
  def mock_keys
    @mock_keys ||= [
      Dash::Key.new(private_key: "Wallet1".sha256),
      Dash::Key.new(private_key: "Wallet2".sha256)
    ]
  end
  ...
  def mock_utxos
    ...
    (0...32).map do |i|
      Dash::TransactionOutput.new(value:  100_000,
