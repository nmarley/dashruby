...
describe Dash::MerkleTree do
  ...
  it "should compute root of one hash equal to that hash" do
    ...
    Dash::MerkleTree.new(hashes: [hash.from_hex]).merkle_root.to_hex.must_equal hash
  end
  ...
  it "merkle root of 2 hashes must equal hash(a+b)" do
    ...
    r.to_hex.must_equal Dash.hash256(a+b).to_hex
    mt = Dash::MerkleTree.new(hashes: [a,b])
  ...
  it "merkle root of 3 hashes must equal Hash(Hash(a+b)+Hash(c+c))" do
    ...
    r.to_hex.must_equal Dash.hash256(Dash.hash256(a+b)+Dash.hash256(c+c)).to_hex
    mt = Dash::MerkleTree.new(hashes: [a,b,c])
    mt.tail_duplicates?.must_equal false
    mt.merkle_root.to_hex.must_equal r.to_hex
  end
