...
if $0 == __FILE__
  ...
  shares.each do |share|
    puts "Share:  #{Dash.to_hex(share)}"
  end
  restored_secret = ssss.restore([shares[1], shares[0]])
  puts "Recovered secret with shares 2 and 1: #{Dash.to_hex(restored_secret)}"
  ...
  test_vectors.each do |test|
    ...
    test.each do |rule, defined_shares|
      ...
      if restored_secret != secret
        failed = true
        puts "Failed #{hexsecret} #{rule} test: failed to restore secret using #{subshares.map{|s| Dash.to_hex(s)}.inspect}"
      end
      if !failed
        puts "Ok."
      end
  ...
  begin
    ...
    (0..255).each do |ver|
      ...
      s = nil
      (0..255).each do |i|
        s = Dash::Base58.base58_from_data(ver.chr + (i.chr*payload_length))
        #lengths[s.length] = 1
        prefixes << s[0,1]
        s = Dash::Base58.base58_from_data(ver.chr + Digest::SHA2.digest(ver.to_s + i.to_s)[0,payload_length])
        lengths[s.length] = 1
        prefixes << s[0,1]
      end
