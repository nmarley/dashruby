...
module Dash
  class Key
    ...
    # Returns a copy of Dash::Key instance with public_key_compressed == true
    attr_reader :compressed_key

    # Returns a copy of Dash::Key instance with public_key_compressed == false
    ...
    def self.random(public_key_compressed: true, network: nil)
      ...
      privkey = Dash::Data.random_data(32)
    ...
    def compressed_public_key
      Dash::OpenSSL.public_key_with_compression(self.public_key, true)
    end

    def uncompressed_public_key
      Dash::OpenSSL.public_key_with_compression(self.public_key, false)
    end
    ...
    # returns resulting curve point as Dash::Key object (pubkey only).
    ...
    def diffie_hellman(key2)
      ...
      lib.autorelease do |pool|
        ...
        lib.EC_POINT_point2bn(
          ...
          Dash::OpenSSL::POINT_CONVERSION_COMPRESSED :
          Dash::OpenSSL::POINT_CONVERSION_UNCOMPRESSED,
          pubkey_x,
          pool.bn_ctx
        )
    ...
    def ecdsa_signature(hash, normalized: true)
      Dash::OpenSSL.ecdsa_signature(hash, @private_key, normalized: normalized)
    end

    # Returns true if ECDSA signature is valid for a given hash
    def verify_ecdsa_signature(signature, hash)
      Dash::OpenSSL.ecdsa_verify(signature, hash, self.public_key)
    end

    def self.normalized_signature(signature)
      Dash::OpenSSL.ecdsa_normalized_signature(signature)
    end
    ...
    def self.validate_script_signature(data, verify_lower_s: true, verify_hashtype: true)
      ...
      data = Dash::Data.ensure_binary_encoding(data) # so we can use #[] on byte level.
    ...
    def self.compare_big_endian(s1, s2)
      s1 = Dash::Data.ensure_binary_encoding(s1)
      s2 = Dash::Data.ensure_binary_encoding(s2)
    ...
    # Private helper to validate portion of a signate. Follows style of dashd.
    ...
    def regenerate_key_pair
      if @private_key
        privkey, pubkey = Dash::OpenSSL.regenerate_keypair(@private_key, public_key_compressed: @public_key_compressed)
        if privkey && pubkey
          @private_key = privkey
          @public_key = pubkey
        end
