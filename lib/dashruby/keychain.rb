...
module Dash
  class Keychain
    ...
    # Default is Dash::Network.default (mainnet if not overriden).
    ...
    def extended_public_key
      @extended_public_key ||= begin
        ...
        Dash::Base58.base58check_from_data(prefix + @public_key)
      end
    end
    ...
    def extended_private_key
      @extended_private_key ||= begin
        ...
        Dash::Base58.base58check_from_data(prefix + "\x00" + @private_key)
      end
    end

    def identifier
      @identifier ||= Dash.hash160(@public_key)
    end

    def identifier_base58
      @identifier_base58 ||= Dash::Base58.base58check_from_data(self.identifier)
    end

    def fingerprint
      @fingerprint ||= Dash::WireFormat.read_uint32be(data:self.identifier).first
    end
    ...
    # Equivalent to Dash::Keychain.new(xpub: keychain.xpub)
    def public_keychain
    ...
    def initialize(seed: nil,
      ...
      if seed
        ...
        if network
          raise ArgumentError, "Cannot use network argument with extended key to initialize Dash::Keychain (network type is already encoded in the key)"
        end
        if [xprv, xpub, extended_key].compact.size != 1
          raise ArgumentError, "Only one of xpub/xprv/extended_key arguments could be used to initialize Dash::Keychain"
        end
    ...
    def init_with_seed(seed, network: nil)
      ...
      @index = 0
      @depth = 0
      @network = network || Dash::Network.default
    end
    private :init_with_seed


    def init_with_extended_key(xkey)
      xkeydata = Dash::Base58.data_from_base58check(xkey)

      if xkeydata.bytesize != 78
        raise DashError, "Invalid extended key length: must be 78 bytes (received #{xkeydata.bytesize} bytes)"
      end

      version, _ = Dash::WireFormat.read_uint32be(data: xkeydata)
      ...
      if version == PRIVATE_MAINNET_VERSION || version == PRIVATE_TESTNET_VERSION
        ...
        if keyprefix != 0x00
          raise DashError, "Extended private key must be padded with 0x00 byte (received #{keyprefix})"
        end
        ...
        if keyprefix == 0x00
          raise DashError, "Extended public key must have non-zero first byte (received #{keyprefix})"
        end
        ...
        raise DashError, "Unknown extended key version: 0x#{version.to_s(16).rjust(8, "0")}"
      end

      @depth = bytes[4] # 0 for master keychain, 1 for first level derived keychain etc.

      @parent_fingerprint, _ = Dash::WireFormat.read_uint32be(data: xkeydata, offset: 5)
      raise DashError, "Cannot read uint32be parent_fingerprint" if !@parent_fingerprint

      @index, _ = Dash::WireFormat.read_uint32be(data: xkeydata, offset: 9)
      raise DashError, "Cannot read uint32be index" if !@index
    ...
    def init_with_components(private_key,
      ...
      @public_key         = public_key || Dash::Key.new(private_key: private_key, public_key_compressed: true).public_key
    ...
    # Index must be less of equal Dash::Keychain::MAX_INDEX, otherwise raises ArgumentError.
    # Raises DashError for some indexes (when hashing leads to invalid EC points)
    ...
    def derived_keychain(index_or_path, hardened: nil)
      ...
      if hardened && !@private_key
        ...
        raise DashError, "Not possible to derive a hardened keychain without a private key (index: #{index})."
      end
      ...
      data << Dash::WireFormat.encode_uint32be(hardened ? (0x80000000 | index) : index)

      digest = Dash.hmac_sha512(data: data, key: @chain_code)
      ...
      lib.autorelease do |pool|
        ...
        if lib.BN_cmp(factor, n) >= 0
          raise DashError, "Factor for index #{index} is greater than curve order."
        end
        ...
        if @private_key
          ...
          if lib.BN_cmp(pk, pool.new_bn("\x00")) == 0
            raise DashError, "Private key is zero for index #{index}."
          end
          ...
          if 1 == lib.EC_POINT_is_at_infinity(lib.group, point)
            raise DashError, "Resulting point is at infinity for index #{index}."
          end

          lib.EC_POINT_point2bn(lib.group, point, Dash::OpenSSL::POINT_CONVERSION_COMPRESSED, pubkey_x, pool.bn_ctx)
    ...
    # Returns a derived Dash::Key from this keychain.
