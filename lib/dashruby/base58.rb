module Dash
  # Base58 is used for compact human-friendly representation of Dash addresses and private keys.
  ...
  module Base58
    ...
    def data_from_base58(string)
      ...
      data = Dash::Data.data_from_bytes(bytes)
      byte_for_1 = "1".bytes.first
      Dash::Data.ensure_ascii_compatible_encoding(string).bytes.each do |byte|
    ...
    def base58check_from_data(data)
      raise ArgumentError, "Data is missing" if !data
      return base58_from_data(data + Dash.hash256(data)[0,4])
    end
    ...
    def data_from_base58check(string)
      ...
      if checksum != Dash.hash256(payload)[0,4]
