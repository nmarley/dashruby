module Dash
  ...
  class TransactionBuilder
    ...
    module Provider
      ...
      # Each unspent output is a Dash::TransactionOutput instance with non-nil `transaction_hash` and `index` attributes.
      ...
      def consume_unspent_output(output)
        #puts "========> DASHRUBY: Provider #{self.object_id} consumes output #{dash_txbuilder_outpoint_id(output)}."
        @dash_txbuilder_provider_consumed_unspent_outputs ||= {}
        @dash_txbuilder_provider_consumed_unspent_outputs[dash_txbuilder_outpoint_id(output)] = 1
        self
      end

      def consumed_unspent_output?(output)
        #puts "========> DASHRUBY: Does Provider #{self.object_id} have output #{dash_txbuilder_outpoint_id(output)}?"
        !!((@dash_txbuilder_provider_consumed_unspent_outputs ||= {})[dash_txbuilder_outpoint_id(output)])
      end

      def dash_txbuilder_outpoint_id(output)
