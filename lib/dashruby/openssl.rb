  ...
  module OpenSSL
    ...
    def public_key_with_compression(pubkey, compressed)
      ...
      autorelease do |pool|
        ...
        if eckey.null?
          raise DashError, "OpenSSL failed to create EC_KEY with public key: #{Dash.to_hex(pubkey).inspect}"
        end
        ...
        if i2o_ECPublicKey(eckey, pointer_to_pointer(buf)) == length
          ...
          raise DashError, "OpenSSL failed to regenerate a public key."
        end
    ...
    def regenerate_keypair(private_key, public_key_compressed: false)
      ...
      autorelease do |pool|
        ...
        if i2d_ECPrivateKey(eckey, pointer_to_pointer(buf)) == length
          ...
          raise DashError, "OpenSSL failed to convert private key to DER format"
        end

        if private_key2 != private_key
          raise DashError, "OpenSSL somehow regenerated a wrong private key."
        end
    ...
    def rfc6979_ecdsa_nonce(hash, privkey)
      ...
      autorelease do |pool|
        ...
        k = Dash.hmac_sha256(key: k, data: v + "\x00".b + privkey + h1data)

        # Step 3.2.e. V = HMAC-SHA256(key: K, data: V)
        v = Dash.hmac_sha256(key: k, data: v)

        # Step 3.2.f. K = HMAC-SHA256(key: K, data: V || 0x01 || int2octets(privkey) || bits2octets(hash))
        k = Dash.hmac_sha256(key: k, data: v + "\x01".b + privkey + h1data)
        ...
        zero32 = "\x00".b*32
        10000.times do
          t = Dash.hmac_sha256(key: k, data: v)
          tn = pool.new_bn(t)
          ...
          k = Dash.hmac_sha256(key: k, data: v + zero32)
          v = Dash.hmac_sha256(key: k, data: v)
        end
    ...
    def ecdsa_reserialize_signature(signature, normalize_s: false)
      ...
      autorelease do |pool|
        ...
        if psig.null?
          Diagnostics.current.add_message("OpenSSL failed to read ECDSA signature with DER during reserialize: #{Dash.to_hex(signature).inspect}")
          return signature
          #raise DashError, "OpenSSL failed to read ECDSA signature with DER: #{Dash.to_hex(signature).inspect}"
        end
    ...
    def ecdsa_verify(signature, hash, public_key)
      ...
      autorelease do |pool|
        ...
        eckey = o2i_ECPublicKey(pointer_to_pointer(eckey), pointer_to_pointer(buf), buf.size - 1)
        if eckey.null?
          Diagnostics.current.add_message("OpenSSL failed to create EC_KEY with public key: #{Dash.to_hex(public_key).inspect}")
          raise DashError, "OpenSSL failed to create EC_KEY with public key: #{Dash.to_hex(public_key).inspect}"
        end
        ...
        if result == 0
          Diagnostics.current.add_message("OpenSSL detected invalid ECDSA signature. Signature: #{Dash.to_hex(signature).inspect}; Hash: #{Dash.to_hex(hash).inspect}; Pubkey: #{Dash.to_hex(public_key).inspect}")
        else
          Diagnostics.current.add_message("OpenSSL failed with error while verifying ECDSA signature. Signature: #{Dash.to_hex(signature).inspect}; Hash: #{Dash.to_hex(hash).inspect}; Pubkey: #{Dash.to_hex(public_key).inspect}; Result: #{result}")
          return false
          # raise DashError, "OpenSSL failed with error while verifying ECDSA signature. Signature: #{Dash.to_hex(signature).inspect}; Hash: #{Dash.to_hex(hash).inspect}; Pubkey: #{Dash.to_hex(public_key).inspect}; Result: #{result}"
        end
        return false
      end
      false
    ...
    def private_key_from_der_format(der_key)
      ...
      if ec_key.null?
        raise DashError, "OpenSSL failed to create EC_KEY with DER private key"
      end
    ...
    def data_from_bn(bn, min_length: nil, required_length: nil)
      ...
      if required_length && s.bytesize != required_length
        raise DashError, "Non-matching length of the number: #{s.bytesize} bytes vs required #{required_length}"
      end
      s
    end
