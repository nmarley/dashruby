...
module Dash
  class TransactionInput
    ...
    # Dash::Script instance that proves ownership of the previous transaction output.
    ...
    def initialize(data: nil,
                   ...
                   coinbase_data: nil,
                   signature_script: Dash::Script.new,
      ...
      if stream || data
        ...
        @previous_hash    = Dash.hash_from_id(previous_id) if previous_id
        @previous_index   = previous_index   || INVALID_INDEX
        @coinbase_data    = coinbase_data    || "".b
        @signature_script = signature_script || Dash::Script.new
    ...
    def init_with_stream(stream)
      ...
      if !(@previous_index = Dash::WireFormat.read_uint32le(stream: stream).first)
      ...
      if is_coinbase
        ...
        @signature_script = Dash::Script.new(data: scriptdata)
      end

      if !(@sequence = Dash::WireFormat.read_uint32le(stream: stream).first)
    ...
    def init_with_dictionary(dict)
      ...
      if dict["prev_out"]
        if hashhex = dict["prev_out"]["hash"]
          prevhash = Dash.from_hex(hashhex)
      ...
      if hex = dict["coinbase"]
        coinbase_data = Dash.from_hex(hex)
      elsif dict["scriptSig"]
        if dict["scriptSig"].is_a?(Hash)
          if hex = dict["scriptSig"]["hex"]
            script = Dash::Script.new(data: Dash.from_hex(hex))
          end
        end
      end
    ...
    def previous_id
      Dash.id_from_hash(previous_hash)
    end
    ...
    def data
      data = "".b
      data << Dash::Data.ensure_binary_encoding(self.previous_hash)
      data << Dash::WireFormat.encode_uint32le(self.previous_index)
      if coinbase?
        data << Dash::WireFormat.encode_string(self.coinbase_data)
      else
        data << Dash::WireFormat.encode_string(self.signature_script.data)
      end
      data << Dash::WireFormat.encode_uint32le(self.sequence)
      data
    end
    ...
    def dictionary
      ...
      dict["prev_out"] = {
        "hash" => Dash.to_hex(self.previous_hash),
        "n"    => self.previous_index
      }

      if self.coinbase?
        dict["coinbase"] = Dash.to_hex(self.coinbase_data)
      else
        dict["scriptSig"] = {
          "asm" => self.signature_script.to_s,
          "hex" => Dash.to_hex(self.signature_script.data)
        }
      end
