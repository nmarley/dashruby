...
# Dash-Qt has later added even more compact format called CVarInt to use in its local block storage. CVarInt is not implemented here.
#
  ...
  module WireFormat
    ...
    def read_varint(data: nil, stream: nil, offset: 0)
      if data && !stream
        ...
        bytes = Dash::Data.bytes_from_data(data, offset: offset, limit: 9) # we don't need more than 9 bytes.
    ...
    def read_string(data: nil, stream: nil, offset: 0)
      if data && !stream
        ...
        string = Dash::Data.ensure_binary_encoding(data)[read_length, string_length]
    ...
    def encode_string(string)
      ...
      encode_varint(string.bytesize) + Dash::Data.ensure_binary_encoding(string)
    end
    ...
    def write_string(string, data: nil, stream: nil)
      ...
      stringbuf = Dash::Data.ensure_binary_encoding(string)
    ...
    def _read_fixint(name: nil, length: nil, pack_format: nil, data: nil, stream: nil, offset: 0)
      if data && !stream
        ...
        if data.bytesize < offset + length
          Diagnostics.current.add_message("Dash::WireFormat#read_#{name}: Not enough bytes to read #{name} in binary string.")
          return [nil, 0]
        end
        ...
        if !buf
          Diagnostics.current.add_message("Dash::WireFormat#read_#{name}: Failed to read #{name} from stream.")
          return [nil, offset]
        end

        if buf.bytesize < length
          Diagnostics.current.add_message("Dash::WireFormat#read_#{name}: Not enough bytes to read #{name} from stream.")
        ...
        raise ArgumentError, "Dash::WireFormat#read_#{name}: Either data or stream must be specified."
      end
    end
