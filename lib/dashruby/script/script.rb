module Dash
  class Script
    ...
    def initialize(hex: nil, # raw script data in hex
      ...
      if data || hex
        data ||= Dash.from_hex(hex)
        data = Dash::Data.ensure_binary_encoding(data)
        @chunks = []
        offset = 0
        while offset < data.bytesize
          chunk = ScriptChunk.with_data(data, offset: offset)
          if !chunk.canonical?
            Diagnostics.current.add_message("Dash::Script: decoded non-canonical chunk at offset #{offset}: #{chunk.to_s}")
          end
          offset += chunk.size
          @chunks << chunk
        end
    ...
    def standard_multisig_script?
      ...
      # Dash allows encoding multisig N and M parameters as pushdata
    ...
    # (including OP_<N>). Aka isPushOnly in Dash-Qt.
    ...
    # Returns Dash::PublicKeyAddress or Dash::ScriptHashAddress if
    ...
    def standard_address(network: nil)
      if public_key_hash_script?
        return Dash::PublicKeyAddress.new(hash: @chunks[2].pushdata, network: network)
      elsif script_hash_script?
        return Dash::ScriptHashAddress.new(hash: @chunks[1].pushdata, network: network)
      elsif public_key_script?
        return Dash::PublicKeyAddress.new(hash: Dash.hash160(@chunks[0].pushdata), network: network)
      end
      nil
    end
    ...
    def p2sh_script
      self.class.new << OP_HASH160 << Dash.hash160(self.data) << OP_EQUAL
    end
    ...
    def append(object)
      if object.is_a?(Dash::Script)
        append_script(object)
      elsif object.is_a?(Dash::ScriptNumber)
        ...
        raise ArgumentError, "Operand must be an integer, a string a Dash::Script instance or an array of any of those."
      end
      return self
    end
    ...
    def find_and_delete(subscript)
      subscript.is_a?(Script) or raise ArgumentError,"Argument must be an instance of Dash::Script"
      # Replacing [a b a]
    ...
    def self.encode_pushdata(pushdata, opcode: nil)
      ...
      if pushdata.bytesize < OP_PUSHDATA1 && opcode == nil
        return Dash::WireFormat.encode_uint8(pushdata.bytesize) + pushdata
      elsif pushdata.bytesize < 0xff && (opcode == nil || opcode == OP_PUSHDATA1)
        return Dash::WireFormat.encode_uint8(OP_PUSHDATA1) +
               Dash::WireFormat.encode_uint8(pushdata.bytesize) +
               pushdata
      elsif pushdata.bytesize < 0xffff && (opcode == nil || opcode == OP_PUSHDATA2)
        return Dash::WireFormat.encode_uint8(OP_PUSHDATA2) +
               Dash::WireFormat.encode_uint16le(pushdata.bytesize) +
               pushdata
      elsif pushdata.bytesize < 0xffffffff && (opcode == nil || opcode == OP_PUSHDATA4)
         return Dash::WireFormat.encode_uint8(OP_PUSHDATA4) +
                Dash::WireFormat.encode_uint32le(pushdata.bytesize) +
                pushdata
      else
    ...
    def detect_multisig
      ...
      if !m
        ...
        m = Dash::BigNumber.new(signed_little_endian: m_chunk.pushdata).integer
      end

      if !n
        return false if !n_chunk.pushdata?
        n = Dash::BigNumber.new(signed_little_endian: n_chunk.pushdata).integer
      end
