...
module Dash
  ...
  class ScriptInterpreter
      ...
      while chunk = @run_script_chunks.shift
        ...
        if should_execute && 0 <= opcode && opcode <= OP_PUSHDATA4
          ...
          when OP_CHECKSIG, OP_CHECKSIGVERIFY
            ...
            subscript = subscript.find_and_delete(Dash::Script.new << sig)
          ...
          when OP_CHECKMULTISIG, OP_CHECKMULTISIGVERIFY
            ...
            while (i-=1) > 0  # Dash Core: while (i-- > 1)
              stack_pop
            end
    ...
    def valid_signature_encoding(sig)
      Dash::Key.validate_script_signature(sig, verify_lower_s: false, verify_hashtype: false)
    end

    def low_der_signature(sig)
      Dash::Key.validate_script_signature(sig, verify_lower_s: true, verify_hashtype: false)
    end

    def defined_hashtype_signature(sig)
      Dash::Key.validate_script_signature(sig, verify_lower_s: false, verify_hashtype: true)
    end

    def compressed_or_uncompressed_pubkey(pubkey)
      Dash::Key.validate_public_key(pubkey)
    end
