  ...
  class BlockHeader
    ...
    def self.genesis_mainnet
      self.new(
        ...
        merkle_root:         Dash.from_hex("c762a6567f3cc092f0684bb62b7e00a84890b990f07cc71a6bb58d64b98e02e0"),
    ...
    def initialize(data: nil,
      ...
      if stream || data
        ...
        @previous_block_hash = Dash.hash_from_id(previous_block_id) if previous_block_id
    ...
    def init_with_stream(stream)
      ...
      if !(timestamp = Dash::WireFormat.read_uint32le(stream: stream).first)
        raise ArgumentError, "Failed to read 32-byte block timestamp from the stream."
      end

      if !(bits = Dash::WireFormat.read_uint32le(stream: stream).first)
        raise ArgumentError, "Failed to read 32-byte proof-of-work bits from the stream."
      end

      if !(nonce = Dash::WireFormat.read_uint32le(stream: stream).first)
    ...
    def previous_block_id
      Dash.id_from_hash(self.previous_block_hash)
    end

    def previous_block_id=(block_id)
      self.previous_block_hash = Dash.hash_from_id(block_id)
    end
    ...
    def header_data # so that in subclass Block we don't hash the entire block
      ...
      data << Dash::WireFormat.encode_uint32le(self.timestamp)
      data << Dash::WireFormat.encode_uint32le(self.bits)
      data << Dash::WireFormat.encode_uint32le(self.nonce)
      data
    end
    ...
    def inspect
      ...
      %{ merkle_root:#{Dash.id_from_hash(self.merkle_root)[0,16]}} +
      %{ timestamp:#{self.timestamp}} +
