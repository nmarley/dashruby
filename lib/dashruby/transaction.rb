# Transaction represents a Dash transfer from one or more inputs to one or more outputs.
require 'stringio'
module Dash
  ...
  class Transaction
    ...
    DEFAULT_FEE_RATE = 10_000 # duffs per 1000 bytes
    DEFAULT_RELAY_FEE_RATE = 1000 # duffs per 1000 bytes
    ...
    def init_with_stream(stream)
      ...
      if !(version = Dash::WireFormat.read_int32le(stream: stream).first)
        raise ArgumentError, "Failed to read version prefix from the stream."
      end

      if !(inputs_count = Dash::WireFormat.read_varint(stream: stream).first)
      ...
      if !(outputs_count = Dash::WireFormat.read_varint(stream: stream).first)
      ...
      if !(lock_time = Dash::WireFormat.read_uint32le(stream: stream).first)
    ...
    def data
      data = "".b
      data << Dash::WireFormat.encode_int32le(self.version)
      data << Dash::WireFormat.encode_varint(self.inputs.size)
      self.inputs.each do |txin|
        data << txin.data
      end
      data << Dash::WireFormat.encode_varint(self.outputs.size)
      self.outputs.each do |txout|
        data << txout.data
      end
      data << Dash::WireFormat.encode_uint32le(self.lock_time)
      data
    end
    ...
    def signature_hash(input_index: nil, output_script: nil, hash_type: Dash::SIGHASH_ALL, version: 0, amount: 0)
      ...
      # Note: Dash-Qt returns a 256-bit little-endian number 1 in such case,
      ...
      # but that's what Dash-Qt does as well.
      output_script.delete_opcode(Dash::OP_CODESEPARATOR)
      ...
      tx.inputs.each do |txin|
        txin.signature_script = Dash::Script.new
      end
      tx.inputs[input_index].signature_script = output_script

      # Blank out some of the outputs depending on DashSignatureHashType
      # Default is SIGHASH_ALL - all inputs and outputs are signed.
      if (hash_type & Dash::SIGHASH_OUTPUT_MASK) == Dash::SIGHASH_NONE
      ...
      # Outputs before the one we need are blanked out. All outputs after are simply removed.
      elsif (hash_type & Dash::SIGHASH_OUTPUT_MASK) == Dash::SIGHASH_SINGLE
        # Only lock-in the txout payee at same index as txin.
        output_index = input_index;

        # If output_index is out of bounds, Dash-Qt is returning a 256-bit little-endian 0x01 instead of failing with error.
        ...
        (0...output_index).each do |i|
          tx.add_output(Dash::TransactionOutput.new)
        end
      ...
      if (hash_type & Dash::SIGHASH_ANYONECANPAY) != 0
      ...
      data = tx.data + Dash::WireFormat.encode_uint32le(hash_type)
      hash = Dash.hash256(data)
      # puts ""
      # puts "SIGHASH[#{self.transaction_id}, input #{input_index}, hashtype 0x#{hash_type.to_s(16)}]: hash = #{Dash.id_from_hash(hash)}; tx = " + tx.inspect
      # puts ""
      return hash
    end
    ...
    def self.compute_fee(size, fee_rate: DEFAULT_FEE_RATE)
      ...
      fee = fee_rate*size/1000 # according to Dash as of ?
