module Dash
  ...
  class Block < BlockHeader
    ...
    # Array of Dash::Transaction objects
    ...
    def self.genesis_mainnet
      self.new(
        ...
        bits:                0x1e0ffff0,
        nonce:               0x1b93fc2,
        transactions:        [Dash::Transaction.new(
          version:   1,
          inputs:    [
            Dash::TransactionInput.new(
              coinbase_data: Dash.from_hex("04FFFF001D01044C595769726564" +
          ...
          outputs:   [
            Dash::TransactionOutput.new(
              value: 50*COIN,
              script: Script.new(data: Dash.from_hex("41040184710FA689AD" +
    ...
    def init_with_stream(stream)
      super(stream)
      if !(txs_count = Dash::WireFormat.read_varint(stream: stream).first)
    ...
    def data
      data = super
      data << Dash::WireFormat.encode_varint(self.transactions.size)
