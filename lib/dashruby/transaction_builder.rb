...
module Dash
  ...
  class TransactionBuilder
    ...
    # An array of Dash::TransactionOutput instances specifying how many coins to spend and how.
    # If the array is empty, all unspent outputs are spent to the change address.
    attr_accessor :outputs

    # Change address (base58-encoded string or Dash::Address).
    ...
    # Could be base58-encoded address or Dash::Address instances.
    ...
    # Actual available Dash::TransactionOutput's to spend.
    ...
    def input_addresses=(addresses)
      ...
      # Normalize addresses to make them all Dash::Address instances
      if @input_addresses
        @input_addresses = @input_addresses.map do |addr|
          Dash::Address.parse(addr).tap do |a|
            if !a.is_a?(Dash::DashPaymentAddress) && !a.is_a?(WIF)
              raise ArgumentError, "Expected DashPaymentAddress and WIF instances"
            end
    ...
    def build
      ...
      if (self.outputs || []).size == 0
        ...
        if result.outputs_amount < result.fee
          Diagnostics.current.add_message("Dash::TransactionBuilder: Warning! Spending all unspent outputs returns less than a mining fee. (#{result.outputs_amount} < #{result.fee})")
        end
      ...
      grouped_inputs.each do |script_data, inputs|
        ...
        if script.p2pk? || script.p2pkh?
          ...
          if !key && (key = self.signer ? self.signer.signing_key_for_output(output: first_input.transaction_output, address: address) : nil)
            if !key.private_key
              raise ArgumentError, "Provided Dash::Key must contain the private key"
            end
        ...
        if key
          ...
          inputs.each do |txin|
            ...
            if script.p2pk?
              ...
              Dash::Invariant(false, "All txins are expected to be of the same type")
