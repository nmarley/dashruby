module Dash
  ...
  class WIF < Address
    ...
    def key
      Dash::Key.new(private_key: self.private_key, public_key_compressed: @public_key_compressed, network: self.network)
    end
    ...
    def data_for_base58check_encoding
      data = Dash::Data.data_from_bytes([self.version]) + @data
      if @public_key_compressed
        data += Dash::Data.data_from_bytes([0x01])
      end
      return data
    end

    def inspect
      %{#<#{self.class}:#{to_s} privkey:#{Dash.to_hex(data)} (#{@public_key_compressed ? '' : 'un'}compressed pubkey)>}
    end
